...
public class ServiceSet {
  ...
  public static ServiceSet loadServiceSet(String name) throws IOException {
    ...
    if (template == null) {
      throw new IllegalArgumentException("System service not found: " + name);
    }
      ...
      for (String filename : list) {
        ...
        if (
          ...
          try {
            ...
            if (logger.isLoggable(Level.FINE)) {
              logger.fine("Skipping not int parseable: " + filename);
            }
          }
        }
      }
  ...
  public static ServiceSet loadServiceSet(Service template) throws IOException {
    ...
    if (logger.isLoggable(Level.FINE)) {
      logger.fine("Loading service set: " + templateName);
    }
    ...
    for (Map.Entry<String, File> entry : servicesToLoad.entrySet()) {
      ...
      if (service != null) {
        if (logger.isLoggable(Level.FINE)) {
          logger.fine("Successfully loaded service: " + service + " from " + file);
        }
        if (!services.add(service)) {
          throw new AssertionError("Duplicate service: " + service);
        }
      } else {
        if (logger.isLoggable(Level.FINE)) {
  ...
  private static final Comparator<SortedSet<ProtocolOrPortRange>> portSetComparator = (SortedSet<ProtocolOrPortRange> ports1, SortedSet<ProtocolOrPortRange> ports2) -> {
    ...
    while (iter1.hasNext()) {
      if (!iter2.hasNext()) {
        // ports2 shorter list than ports1
        return 1;
      }
    ...
    if (iter2.hasNext()) {
      // ports1 shorter list than ports2
      return -1;
    }
    return 0;
  };
  ...
  public static ServiceSet createOptimizedServiceSet(Service template, Iterable<? extends Target> targets) {
    ...
    for (Map.Entry<SortedSet<ProtocolOrPortRange>, SortedSet<InetAddressPrefix>> entry : coalescedDestinationsByPorts.entrySet()) {
      ...
      for (InetAddressPrefix destinationToSplit : destinationsToSplit) {
        ...
        if (!destinationsForFamily.add(destinationToSplit)) {
          throw new AssertionError();
        }
      }
    }
    ...
    for (Map.Entry<SortedSet<ProtocolOrPortRange>, Map<ProtocolFamily, SortedSet<InetAddressPrefix>>> entry : splitByFamily.entrySet()) {
      ...
      if (ipv4Destinations == null) {
        ipv4Iter = Collections.emptyIterator();
      } else {
        ipv4Iter = ipv4Destinations.iterator();
      }
      Iterator<InetAddressPrefix> ipv6Iter;
      if (ipv6Destinations == null) {
        ipv6Iter = Collections.emptyIterator();
      } else {
        ipv6Iter = ipv6Destinations.iterator();
      }
    ...
    if (logger.isLoggable(Level.FINE)) {
      logger.fine("Finished " + services.size() + (services.size() == 1 ? " service." : " services."));
    }
    return new ServiceSet(template, services);
  }
  ...
  public static void commit(Iterable<ServiceSet> serviceSets, Set<String> zones) throws IOException {
    ...
    for (ServiceSet serviceSet : serviceSets) {
      ...
      if (serviceSetsMap.put(name, serviceSet) != null) {
    ...
    synchronized (Firewalld.firewallCmdLock) {
      ...
      for (ServiceSet serviceSet : serviceSetsMap.values()) {
        for (Service service : serviceSet.services) {
          ...
          if (!serviceNames.add(serviceName)) {
            throw new AssertionError("Duplicate service name: " + serviceName);
          }
        }
      }
          ...
          if (serviceName.equals(templateName)) {
            ...
            if (systemService != null && service.equals(systemService)) {
              ...
              if (serviceFile.exists()) {
                if (logger.isLoggable(Level.FINE)) {
                  logger.fine("Deleting local service file handled by system file: " + serviceFile);
                }
                Files.delete(serviceFile.toPath());
                needsReload = true;
              }
      ...
      for (String zone : zones) {
        ...
        if (servicesForZone == null) {
          throw new IOException("Zone not found: " + zone);
        }
        Set<String> toAdd = new TreeSet<>();
        for (ServiceSet serviceSet : serviceSetsMap.values()) {
          for (Service service : serviceSet.services) {
            String serviceName = service.getName();
            if (!servicesForZone.contains(serviceName)) {
              toAdd.add(serviceName);
            }
          }
        }
        if (!toAdd.isEmpty()) {
